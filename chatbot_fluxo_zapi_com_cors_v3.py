...
print("🟢 Versão ATIVA: chatbot_fluxo_zapi_com_cors_v3.py iniciada com sucesso", flush=True)
...
def webhook():
    print(f"📩 Cabeçalhos recebidos: {dict(request.headers)}", flush=True)
    print(f"Raw body: {request.data}", flush=True)
    try:
        data = request.get_json()
        print(f"📦 Payload interpretado: {data}", flush=True)
        print(f"📐 Tipo do payload: {type(data)}", flush=True)
    except Exception as e:
        print(f"❌ Erro ao interpretar JSON: {e}", flush=True)
        return jsonify({"status": "erro no json"})
    ...
    if not incoming_msg or not from_number:
        print("❗ Ignorado: mensagem ou número ausente", flush=True)
    ...
    def send_message(text):
        ...
        print(f"[Z-API RESPONSE] {response.status_code} - {response.text}", flush=True)

    print(f"[RECEBIDO] De {from_number} - Etapa {step}: {incoming_msg}", flush=True)
