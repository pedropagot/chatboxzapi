...
ZAPI_INSTANCE_URL = os.environ.get("ZAPI_INSTANCE_URL")  # Ex: https://api.z-api.io/instances/xxxx/token/yyyy
...
def webhook():
    ...
    incoming_msg = data.get("message", {}).get("body", "").strip().lower()
    from_number = data.get("contact", {}).get("number", "")
