...
def webhook():
    ...
    # Captura dos dados no formato Z-API
    ...
    if not user_id:
        return jsonify({"status": "erro", "mensagem": "Número do usuário não fornecido"}), 400

    if not isinstance(message_raw, str):
        return jsonify({"status": "erro", "mensagem": "Formato da mensagem inválido"}), 400
